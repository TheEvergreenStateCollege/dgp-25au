...
public class GameHelper {
    ...
    /**
     * Read a line of user input from standard input (stdin)
     * @param prompt
     * @return
     */
    ...
    public ArrayList<String> placeStartup(int startupSize) {
        ...
        while (!success && attempts < MAX_ATTEMPTS) {
            ...
            for (int i = 0; i < startupCoords.length; i += 1) {
                startupCoords[i] = location;
                location += increment;
            }
            // System.out.println("Trying: " + Arrays.toString())
    ...
    private void savePositionToGrid(int[] startupCoords) {
        for (int coord : startupCoords) {
            grid[coord] = 1; // set grid position as used
        }
    }
    ...
    private String getAlphaCoordsFromIndex(int index) {
        ...
        return ALPHABET.indexOf(row) + Integer.valueOf(col).toString();
    }
